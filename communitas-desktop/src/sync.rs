...
mod tests {
    ...
    fn fec_roundtrip_recovers_with_missing_shares() {
        ...
        // reed-solomon-simd v3 only supports recovery with missing parity shards
        // Keep all k data shards (0..k), drop m parity shards (k..k+m)
        let mut provided: Vec<Option<Vec<u8>>> = shares.into_iter().map(Some).collect();
        provided[4] = None; // Remove parity shard at index 4
        provided[5] = None; // Remove parity shard at index 5

        let decoded = codec.decode(&provided).unwrap();
        // Reed-Solomon pads data to shard size, so trim to original length
