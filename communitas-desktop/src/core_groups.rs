...
pub async fn core_group_create(
    ...
    // Store group signing key for future membership updates
    {
        ...
        if let Some(ref mut ctx_mut) = *w {
            ctx_mut
                .group_keys
                .insert(hex::encode(packet.id.as_bytes()), kp);
        }
    }
...
pub async fn core_group_add_member(
    ...
    let member_id =
        fw_to_key(member_words.clone()).map_err(|e| format!("fw_to_key member: {}", e))?;
    ...
    if !gip.members.iter().any(|m| m.member_id == member_id) {
        gip.members.push(MemberRef {
            member_id: member_id.clone(),
            member_pk: member_pkt.pk.clone(),
        });
    }
    ...
    let ml = MlDsa65::new();
    let sig = ml
        .sign(&kp.group_sk, &msg)
        .map_err(|e| format!("group sign failed: {:?}", e))?;
