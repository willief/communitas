...
// Useful when clients can’t use stdio transports.
...
server.registerTool(
  'browser_fill_by_label',
  {
    ...
    description: 'Find input/textarea associated with a label containing text and fill value',
  ...
  async ({ label, value }) => {
    ...
    const ok = await page.evaluate((lab, val) => {
      function findByLabelText(text) {
        ...
        if (!el) {
          el = match.querySelector('input,textarea');
        }
        if (!el) {
