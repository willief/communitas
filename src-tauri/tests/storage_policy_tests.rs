...
mod storage_policy_tests {
    ...
    use crate::storage::{
        StoragePolicy, PolicyManager, EncryptionMode, DeduplicationScope,
        StorageError, NamespaceManager, GroupManager
    };
    ...
    fn test_storage_policy_creation() {
        ...
        assert_eq!(private_max.encryption_mode(), EncryptionMode::ChaCha20Poly1305Local);
    ...
    fn test_private_scoped_policy() {
        ...
        assert_eq!(policy.encryption_mode(), EncryptionMode::ChaCha20Poly1305Derived);
        assert_eq!(policy.deduplication_scope(), DeduplicationScope::User(namespace));
    ...
    fn test_group_scoped_policy() {
        ...
        let policy = StoragePolicy::GroupScoped { group_id: group_id.clone() };
        
        assert_eq!(policy.encryption_mode(), EncryptionMode::ChaCha20Poly1305Shared);
        assert_eq!(policy.deduplication_scope(), DeduplicationScope::Group(group_id));
        assert!(policy.allows_sharing());
    ...
    fn test_policy_validation() {
        ...
        assert!(manager.validate_content(&valid_policy, &oversized_content).is_err());
    }
    ...
    fn test_namespace_validation() {
        ...
        for namespace in valid_namespaces {
            assert!(manager.validate_namespace(namespace).is_ok(), 
                   "Valid namespace '{}' should pass validation", namespace);
        }
        ...
        for namespace in invalid_namespaces {
            assert!(manager.validate_namespace(namespace).is_err(),
                   "Invalid namespace '{}' should fail validation", namespace);
        }
    }
    ...
    fn test_content_type_validation() {
        ...
        assert!(manager.validate_content_type(&markdown_policy, markdown_content).is_ok());
        
        // Binary content should fail for PublicMarkdown
        let binary_content = vec![0xFF, 0xD8, 0xFF, 0xE0]; // JPEG header
        assert!(manager.validate_content_type(&markdown_policy, &binary_content).is_err());
    }
    ...
    async fn test_group_access_validation() {
        ...
        match result.unwrap_err() {
            StorageError::AccessDenied { group_id: g, user_id: u } => {
    ...
    fn test_namespace_isolation() {
        ...
        assert_ne!(alice_encrypted.content_address, bob_encrypted.content_address);
    }
    ...
    async fn test_group_key_sharing() {
        ...
        let alice_key = group_manager.get_group_key(group_id, "alice").await.unwrap();
    ...
    async fn test_policy_transitions() {
        ...
        let result = manager.transition_policy(content_address, &from_policy, &to_policy).await;
        assert!(result.is_ok());
    ...
    mod benchmarks {
        ...
        fn bench_content_encryption(c: &mut Criterion) {
            ...
            c.bench_function("content_encryption_1kb", |b| {
                b.iter(|| {
                    manager.encrypt_content(&policy, black_box(&content)).unwrap()
                })
            });
        }
        ...
        fn bench_content_addressing(c: &mut Criterion) {
            ...
            c.bench_function("content_addressing_1mb", |b| {
                b.iter(|| {
                    generate_content_address(black_box(&content))
                })
            });
        }

        criterion_group!(benches, bench_key_derivation, bench_content_encryption, bench_content_addressing);
    ...
    fn test_namespace_collision_resistance() {
        ...
        for i in 0..10000 {
            ...
            assert!(!seen_keys.contains(&key), "Key collision detected for namespace: {}", namespace);
    ...
    async fn test_concurrent_operations() {
        ...
        for i in 0..100 {
            ...
            let handle = tokio::spawn(async move {
                let encrypted = manager_clone.encrypt_content(&policy_clone, &content_clone).unwrap();
                manager_clone.decrypt_content(&policy_clone, &encrypted).unwrap()
            });
