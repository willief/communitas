...
async fn main() {
    ...
    match &keys_result {
        Ok(keys) => {
            ...
            let master_key_ok = keys.get("masterKey").and_then(|v| v.as_str()) == Some("test_master_key");
            ...
            match migration_manager.migrate_from_file_storage(&temp_dir).await {
                Ok(true) => {
                    ...
                    match migration_manager.get_encryption_keys().await {
                        Ok(migrated_keys) => {
                            let master_ok = migrated_keys.get("masterKey").and_then(|v| v.as_str()) == Some("file_master_key");
                            let pair_ok = migrated_keys.get("keyPair").and_then(|v| v.as_str()) == Some("file_key_pair");
