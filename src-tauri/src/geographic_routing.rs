...
use saorsa_core::{
    dht::{
        ...
        geographic_network_integration::{
            GeographicNetworkIntegration, DhtOperationType,
        },
    },
...
impl GeographicRoutingManager {
    ...
    pub async fn new(local_region: Option<GeographicRegion>) -> Result<Self> {
        ...
        info!("Initializing Geographic Routing Manager for region: {:?}", region);
        ...
        let network_integration = Arc::new(
            GeographicNetworkIntegration::new(region)?
        );
    ...
    pub async fn add_peer(&self, peer_id: String, address: String) -> Result<()> {
        debug!("Adding peer {} at {} to geographic routing", peer_id, address);
        
        // Parse address to Multiaddr
        let multiaddr: Multiaddr = address.parse()?;
        
        // Add to network service
        self.network_service.write().await
            .add_peer(peer_id.clone(), multiaddr.clone()).await?;
        ...
        self.peer_info_cache.write().await.insert(peer_id, peer_info);
    ...
    pub async fn update_peer_quality(
        ...
        self.network_service.write().await
            .update_peer_quality(peer_id, success, latency).await?;
        ...
        if let Some(peer_info) = self.peer_info_cache.write().await.get_mut(peer_id) {
            ...
            peer_info.connection_quality = match (peer_info.latency_ms, peer_info.reliability_score) {
    ...
    pub async fn get_all_peers(&self) -> Vec<GeographicPeerInfo> {
        self.peer_info_cache.read().await.values().cloned().collect()
    }
    ...
    pub async fn get_network_overview(&self) -> Result<GeographicNetworkOverview> {
        ...
        for peer in peers.values() {
            *regional_distribution.entry(peer.region.clone()).or_insert(0) += 1;
            ...
            if let Some(latency) = peer.latency_ms {
                let latencies = avg_regional_latency.entry(peer.region.clone())
        ...
        Ok(GeographicNetworkOverview {
            ...
            routing_efficiency,
            geographic_coverage: peers.values()
    ...
    pub async fn select_peers_for_operation(
        ...
        self.network_service.read().await
