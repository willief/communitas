...
pub async fn generate_four_word_identity(
    ...
    // Security: Generate 4 cryptographically secure random words
    let words = if let Some(seed_str) = seed {
        ...
        // Security: Use cryptographically secure deterministic word selection
        let mut selected_words = Vec::new();
        for i in 0..4 {
            let byte_index = i * 8;
            if byte_index + 8 <= hash_bytes.len() {
                let word_bytes = &hash_bytes[byte_index..byte_index + 8];
                let _word_index = u64::from_le_bytes(word_bytes.try_into().map_err(|_| "Invalid word index bytes".to_string())?);
                
                // Security: For deterministic generation, we still use random words
                // but seeded with the hash to ensure cryptographic quality
        ...
        // Security: Use OsRng for cryptographically secure random generation
...
pub async fn check_identity_availability(
    ...
    let claimed = state.claimed.lock().map_err(|_| "Claimed map lock poisoned".to_string())?;
...
pub async fn generate_identity_keypair() -> Result<(String, String), String> {
    ...
    Ok((hex::encode(signing_key.to_bytes()), hex::encode(verifying_key.to_bytes())))
}
...
pub async fn claim_four_word_identity_with_proof(
    ...
    // Create Ed25519 signing key and sign the four-words to prove ownership
    ...
    let mut claimed = state.claimed.lock().map_err(|_| "Claimed map lock poisoned".to_string())?;
...
pub async fn verify_identity_packet(
    ...
    match VerifyingKey::from_bytes(
        ...
        Ok(verifying_key) => {
            let signature_bytes: [u8; 64] = identity_packet.signature.try_into()
                .map_err(|_| "Invalid signature length".to_string())?;
            
            let signature = Signature::from_bytes(&signature_bytes);
            match verifying_key.verify(identity_packet.four_words.as_bytes(), &signature) {
                Ok(()) => Ok(true),
                Err(_) => Ok(false),
            }
        }
...
pub async fn publish_identity_packet(
    ...
    // TODO: Implement DHT publishing using saorsa-core Network API when P2P node is available
    // For now, we store locally as a fallback
    if let Some(_p2p) = &app_state.read().await.p2p_node {
        // TODO: Use saorsa-core Network.store() method to publish to DHT
        // let key = packet.dht_id.clone();
        // let value = serde_json::to_vec(&packet).map_err(|e| e.to_string())?;
        // network.store(key, value).await.map_err(|e| e.to_string())?;
    }
...
mod tests {
    ...
    async fn test_signature_verification() {
        ...
        let signing_key = SigningKey::from_bytes(&private_key_bytes.try_into().expect("Invalid private key length"));
        let public_key = signing_key.verifying_key();
        ...
        let invalid_packet = IdentityPacket {
