...
use saorsa_pqc::api::{ChaCha20Poly1305, MlKemVariant, ml_kem_768};
...
impl MlKemEncapsulation {
    ...
    pub fn encapsulate(public_key_bytes: &[u8]) -> StorageResult<Self> {
        let kem = ml_kem_768();
        let public_key =
            saorsa_pqc::api::MlKemPublicKey::from_bytes(MlKemVariant::MlKem768, public_key_bytes)
        ...
        let (shared_secret, ciphertext) =
            kem.encapsulate(&public_key)
                .map_err(|_| StorageError::Encryption {
                    source:
                        crate::saorsa_storage::errors::EncryptionError::CipherInitializationFailed,
                })?;
    ...
    pub fn decapsulate(
        ...
        let kem = ml_kem_768();
        let secret_key =
            saorsa_pqc::api::MlKemSecretKey::from_bytes(MlKemVariant::MlKem768, secret_key_bytes)
                .map_err(|_| StorageError::Encryption {
                source: crate::saorsa_storage::errors::EncryptionError::AuthenticationFailed,
            })?;
        let ciphertext =
            saorsa_pqc::api::MlKemCiphertext::from_bytes(MlKemVariant::MlKem768, ciphertext_bytes)
                .map_err(|_| StorageError::Encryption {
                    source: crate::saorsa_storage::errors::EncryptionError::AuthenticationFailed,
                })?;
        let shared_secret =
...
impl PqcCryptoManager {
    ...
    pub fn new(master_key: [u8; 32]) -> StorageResult<Self> {
        let config = get_config().map_err(|e| StorageError::ConfigError {
    ...
    pub async fn encrypt_content(
        ...
        let encryption_key = self
            .derive_encryption_key(&mode, user_id, namespace, group_id)
            .await?;

        // Generate ML-KEM keypair for this content
        let ml_kem_keypair = self
            .get_or_generate_keypair(&mode, user_id, namespace, group_id)
            .await?;
        ...
        let final_key = self
            .derive_final_key(&encryption_key, &encapsulation.shared_secret, &mode)
            .await?;
    ...
    pub async fn decrypt_content(
        ...
        let final_key = self
            .derive_final_key(&encryption_key, &shared_secret, mode)
            .await?;
    ...
    pub async fn transition_to_pqc(
        ...
        self.encrypt_content(&plaintext, new_policy, user_id, namespace, group_id)
            .await
    }
...
mod tests {
    ...
    async fn test_ml_kem_encapsulation_decapsulation() {
        ...
        let decapsulated_secret =
            MlKemEncapsulation::decapsulate(&encapsulation.ciphertext, &keypair.secret_key)
                .unwrap();
    ...
    async fn test_pqc_crypto_manager_encrypt_decrypt() {
        ...
        let decrypted = manager
            .decrypt_content(&encrypted, "test-user")
            .await
            .unwrap();
    ...
    async fn test_transition_to_pqc() {
        ...
        let legacy_encrypted = manager
            .encrypt_with_chacha20(test_content, &legacy_key)
            .unwrap();

        // Transition to PQC
        let policy = StoragePolicy::PrivateMax;
        let pqc_encrypted = manager
            .transition_to_pqc(
                &legacy_encrypted,
                &legacy_key,
                &policy,
        ...
        let decrypted = manager
            .decrypt_content(&pqc_encrypted, "test-user")
            .await
            .unwrap();
        assert_eq!(decrypted, test_content);
    }
