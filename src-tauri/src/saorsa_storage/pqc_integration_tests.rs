...
use crate::saorsa_storage::engine::{StorageEngine, StorageRequest, RetrievalRequest};
use crate::saorsa_storage::pqc_crypto::{PqcCryptoManager, PqcEncryptionMode, MlKemKeypair, MlKemEncapsulation};
...
impl PqcStorageTestHarness {
    ...
    async fn store_and_retrieve_test(&self, policy: StoragePolicy, namespace: Option<String>, group_id: Option<String>) -> Result<(), StorageError> {
        ...
        assert!(store_response.encrypted_size > self.test_content.len() as u64, 
                "PQC encrypted content should be larger than original");
        assert!(store_response.operation_time_ms >= 0, "Operation time should be non-negative");
        ...
        assert_eq!(retrieval_response.content, self.test_content, 
                   "Retrieved content should match original");
        assert_eq!(retrieval_response.metadata.size, self.test_content.len() as u64,
...
async fn test_ml_kem_keypair_deterministic_generation() {
    ...
    // Verify key sizes
    assert_eq!(keypair1.public_key.len(), crate::saorsa_storage::pqc_crypto::ML_KEM_768_PUBLIC_KEY_SIZE);
    assert_eq!(keypair1.secret_key.len(), crate::saorsa_storage::pqc_crypto::ML_KEM_768_SECRET_KEY_SIZE);
}
...
async fn test_ml_kem_encapsulation_consistency() {
    ...
    assert_eq!(encapsulation.ciphertext.len(), crate::saorsa_storage::pqc_crypto::ML_KEM_768_CIPHERTEXT_SIZE);
...
async fn test_pqc_private_max_storage() {
    ...
    harness.store_and_retrieve_test(
...
async fn test_pqc_private_scoped_storage() {
    ...
    harness.store_and_retrieve_test(
        StoragePolicy::PrivateScoped { namespace: namespace.clone() },
        Some(namespace),
        None,
    ).await.unwrap();
}
...
async fn test_pqc_group_scoped_storage() {
    ...
    harness.store_and_retrieve_test(
        StoragePolicy::GroupScoped { group_id: group_id.clone() },
        None,
        Some(group_id),
    ).await.unwrap();
}
...
async fn test_pqc_public_markdown_storage() {
    ...
    let markdown_request = StorageRequest {
        ...
        metadata: StorageMetadata {
            ...
            tags: vec!["pqc".to_string(), "test".to_string(), "markdown".to_string()],
            created_at: Utc::now(),
            modified_at: None,
            size: 43,
    ...
    // Store content using PQC-enhanced encryption
    let store_response = harness.engine.store_content(markdown_request).await.unwrap();
    
    // Verify storage response
    assert!(store_response.encrypted_size > 43,
            "PQC encrypted content should be larger than original");
    ...
    // Retrieve and verify content
    let retrieval_response = harness.engine.retrieve_content(retrieval_request).await.unwrap();
    
    assert_eq!(retrieval_response.content, b"# Test Markdown\n\nThis is test markdown content.");
}
...
async fn test_pqc_encryption_mode_conversion() {
    ...
    for (legacy_mode, expected_pqc_mode) in test_cases {
        ...
        assert_eq!(converted_mode, expected_pqc_mode,
...
async fn test_pqc_content_encryption_decryption_direct() {
    ...
    let policies = vec![
        StoragePolicy::PrivateMax,
        StoragePolicy::PrivateScoped { namespace: namespace.to_string() },
        StoragePolicy::GroupScoped { group_id: "test_group".to_string() },
        StoragePolicy::PublicMarkdown,
    ];

    for policy in policies {
        let encrypted_content = pqc_manager.encrypt_content(
            test_content,
            &policy,
            user_id,
            Some(namespace),
        ...
        assert_eq!(encrypted_content.ml_kem_ciphertext.len(), 
        ...
        // Decrypt and verify
        let decrypted_content = pqc_manager.decrypt_content(&encrypted_content, user_id).await.unwrap();
...
async fn test_pqc_performance_benchmarks() {
    ...
    let policy = StoragePolicy::PrivateScoped { namespace: "benchmark".to_string() };
    ...
    for size in test_sizes {
        ...
        let encrypted = pqc_manager.encrypt_content(
            &test_content,
            &policy,
            "benchmark_user",
            Some("benchmark"),
        ...
        let decrypted = pqc_manager.decrypt_content(&encrypted, "benchmark_user").await.unwrap();
        ...
        assert!(encrypt_duration.as_millis() < 1000, 
                "Encryption of {}KB should complete in under 1 second", size / 1024);
        assert!(decrypt_duration.as_millis() < 500,
                "Decryption of {}KB should complete in under 500ms", size / 1024);
...
async fn test_pqc_key_derivation_consistency() {
    ...
    let encrypted1 = pqc_manager.encrypt_content(
        test_content, &policy, user_id, Some(namespace), None
    ).await.unwrap();
    
    let encrypted2 = pqc_manager.encrypt_content(
        test_content, &policy, user_id, Some(namespace), None
    ).await.unwrap();
    
    // ML-KEM ciphertexts should be different (probabilistic encryption)
    assert_ne!(encrypted1.ml_kem_ciphertext, encrypted2.ml_kem_ciphertext,
               "ML-KEM ciphertexts should be different due to randomness");
    
    // But both should decrypt correctly
    let decrypted1 = pqc_manager.decrypt_content(&encrypted1, user_id).await.unwrap();
    let decrypted2 = pqc_manager.decrypt_content(&encrypted2, user_id).await.unwrap();
...
async fn test_pqc_error_handling() {
    ...
    let result = MlKemEncapsulation::decapsulate(&valid_encapsulation.ciphertext, &invalid_secret_key);
...
async fn test_pqc_storage_engine_integration() {
    ...
    assert!(store_response.encrypted_size > min_expected_size,
...
async fn test_pqc_multiple_users_same_namespace() {
    ...
    let policy = StoragePolicy::PrivateScoped { namespace: namespace.clone() };
