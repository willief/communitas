...
use crate::geographic_routing::{
    GeographicNetworkOverview, GeographicPeerInfo, GeographicRoutingManager, GeographicStatus,
    RegionalPeerStats,
};
use anyhow::Result;
...
pub async fn update_peer_quality(
    ...
    if let Some(geo_manager) = manager_guard.as_ref() {
        geo_manager
            .update_peer_quality(&peer_id, success, latency_ms)
            .await
...
pub async fn select_optimal_peers(
    ...
    if let Some(geo_manager) = manager_guard.as_ref() {
        geo_manager
            .select_peers_for_operation(&operation, count)
            .await
...
pub async fn update_geographic_config(
    ...
    if config.enabled {
        ...
        if manager_guard.is_none() {
            ...
            let local_region = config.local_region.and_then(|r| match r.as_str() {
                "NorthAmerica" => {
                    Some(saorsa_core::dht::geographic_routing::GeographicRegion::NorthAmerica)
                }
                "Europe" => Some(saorsa_core::dht::geographic_routing::GeographicRegion::Europe),
                "AsiaPacific" => {
                    Some(saorsa_core::dht::geographic_routing::GeographicRegion::AsiaPacific)
                }
                "SouthAmerica" => {
                    Some(saorsa_core::dht::geographic_routing::GeographicRegion::SouthAmerica)
                }
            ...
            let mut new_manager = GeographicRoutingManager::new(local_region)
                .await
