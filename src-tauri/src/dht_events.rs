...
impl DHTEventListener {
    ...
    async fn get_relevant_users(
        ...
        let entity_id = match event {
            ...
            | DHTSyncEvent::OrganizationUpdated { organization } => organization
                .get("id")
                .and_then(|v| v.as_str())
                .map(String::from),
            DHTSyncEvent::OrganizationDeleted { id } => Some(id.clone()),

            DHTSyncEvent::GroupCreated { group } | DHTSyncEvent::GroupUpdated { group } => {
                group.get("id").and_then(|v| v.as_str()).map(String::from)
            }
            DHTSyncEvent::GroupDeleted { id } => Some(id.clone()),

            DHTSyncEvent::ProjectCreated { project } | DHTSyncEvent::ProjectUpdated { project } => {
